...
class ExactSearchMixin:
    ...
    search, which prevents index usage on large tables and makes a pk search
    match substrings of other ids. Filtering the field directly keeps the
    lookup exact and indexed.

    Set `exact_search_field` to the field to match on, and list that same
    field in `search_fields` -- Django only renders the search box when
    `search_fields` is non-empty, but the lookups it would build there are
    replaced wholesale by this mixin.
